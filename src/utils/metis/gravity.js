  ...
  async attachTable(database, tableName, currentTables = null, initialBalance) {
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('address_retrieved', async () => {
        ...
        const callUrl = `${self.jupiter_data.server}/nxt?messageIsPrunable=false&requestType=sendMessage&secretPhrase=${database.passphrase}&recipient=${database.account}&messageToEncrypt=${encryptedData}&feeNQT=${self.jupiter_data.feeNQT}&deadline=${self.jupiter_data.deadline}&recipientPublicKey=${database.publicKey}&compressMessageToEncrypt=true`;
