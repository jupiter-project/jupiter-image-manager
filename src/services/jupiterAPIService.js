...
class JupiterAPIService {
    ...
    constructor(jupiterHost, applicationAccountProperties) {
        ...
        this.appPropsappProps = applicationAccountProperties;
    }
...
// (response.data.errorCode) {}:
     ...
     * @returns {Promise<unknown>}
     */
    async transferMoney(fromJupiterAccount, toJupiterAccount, amount, feeNQT = this.appProps.minimumFee) {
        logger.verbose('#####################################################################################');
        ...
        return new Promise((resolve, reject) => {
            ...
            this.post( {
                ...
                .then(response =>{
                    ...
                    logger.error('Cannot send Jupiter to new accountId, Jupiter issuer has insufficient balance!');
                    return reject(response);
                })
                .catch( error =>{
                    reject(error);
